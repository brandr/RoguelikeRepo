...
public class Scroll extends Readable{
	...
	public final static Effect CURSE_EFFECT = new Effect("Curse",Effect.CURSE,0,0);
	public final static Effect REMOVE_CURSE_EFFECT = new Effect("Remove Curse",Effect.REMOVE_CURSE,0,0);
	public final static Effect ENCHANT_EFFECT=null;		//TODO
	//branding?
	

/*TYPES Of SCROLLS LEFT TO MAKE: (8)

NEXT: cursing or enchanting

	affect Items with stuff that hasn't been implemented yet (5)
...
-evocation
	*I have no idea what this means and it isn't a crawl scroll. have to wait for nick.
	...
	public String plural(Player player){	//TODO: override for each item type
		if(identified)
			return "scrolls of "+scrollType;
		else if(player.itemKnown(this))
			return "scrolls of "+scrollType;
		else
			return "scrolls titled "+scrollText;
	}
	...
	public static void setAllScrollEffects(){
		if(scrollTypeToEffectMap==null){
			...
			scrollTypeToEffectMap.put(CURSE, CURSE_EFFECT);
			scrollTypeToTargetingStyleMap.put(CURSE, new Targeting(Targeting.ITEM_RANDOM,Targeting.CURSEABLE));	
		
			scrollTypeToEffectMap.put(REMOVE_CURSE, REMOVE_CURSE_EFFECT);
			scrollTypeToTargetingStyleMap.put(REMOVE_CURSE, new Targeting(Targeting.ITEM_ALL,Targeting.CURSED));	
		}
	}
	...
	public String getScrollType() {		//what the scroll says after it is identified
		return scrollType;
	}
