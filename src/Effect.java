...
,IMMOLATION
	...
	public String noEffectMessage() {
		switch(effectType){
		case(CURSE):
			return "You feel uneasy for a moment.";
		case(REMOVE_CURSE):
	...
	public void takeEffect(Monster user, Item target){
		...
		case(IDENTIFY):
			...
			break;
		case(CURSE):	//TODO: case for no curseable items.
			if(target.isEquipment())
				((Equipment)target).curse();
			if(user.getClass().equals(Player.class))
				user.changeCurrentMessage("A black glow issues from your "
						+ user.displayItemName(target, false)+"!"
						, user.currentTile, false);
			break;
		case(REMOVE_CURSE):	//TODO: case for no curseable items.
			if(target.isEquipment())
				((Equipment)target).unCurse();
			break;
		}
	}
